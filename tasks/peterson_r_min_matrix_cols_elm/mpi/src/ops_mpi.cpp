...
PetersonRMinMatrixMPI::PetersonRMinMatrixMPI(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput().clear();
}

bool PetersonRMinMatrixMPI::ValidationImpl() {
  return (GetInput() > 0) && (GetOutput().empty());
}

bool PetersonRMinMatrixMPI::PreProcessingImpl() {
  GetOutput().clear();
  GetOutput().reserve(GetInput());
  return true;
}

bool PetersonRMinMatrixMPI::RunImpl() {
...
bool PetersonRMinMatrixMPI::PostProcessingImpl() {
