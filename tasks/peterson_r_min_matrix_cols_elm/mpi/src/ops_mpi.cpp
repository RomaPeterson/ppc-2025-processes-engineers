...
bool PetersonRMinMatrixColsElmMPI::PreProcessingImpl() { return valid_; }
...
bool PetersonRMinMatrixColsElmMPI::RunImpl() {
  ...
  const std::size_t start_row =
      (base_rows * rank_u) + std::min(remainder, rank_u);
  ...
  MPI_Reduce(local_min.data(), global_min.data(), static_cast<int>(n), MPI_INT,
...
bool PetersonRMinMatrixColsElmMPI::PostProcessingImpl() { return true; }
