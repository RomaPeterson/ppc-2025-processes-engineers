...
PetersonRGaussBandHorizontalSchemeMPI::PetersonRGaussBandHorizontalSchemeMPI(
...
void PetersonRGaussBandHorizontalSchemeMPI::SplitRows(
    const InType& matrix, size_t n, size_t cols, int rank, int size, InType& local_matrix,
  ...
  for (size_t i = 0; i < n; ++i) {
    if (static_cast<int>(i) % size == rank) {
      ...
      if (rank == 0) {
        ...
        MPI_Recv(local_matrix[local_index].data(), static_cast<int>(cols), MPI_DOUBLE, 0,
                 static_cast<int>(i), MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
      ++local_index;
    } else if (rank == 0) {
      MPI_Send(matrix[i].data(), static_cast<int>(cols), MPI_DOUBLE,
               static_cast<int>(i) % size, static_cast<int>(i), MPI_COMM_WORLD);
    }
  }
}

bool PetersonRGaussBandHorizontalSchemeMPI::ForwardEliminationMPI(
    InType& local_matrix, const std::vector<int>& map_global_to_local, size_t n, size_t cols,
    int rank, int size) {
...
void PetersonRGaussBandHorizontalSchemeMPI::EliminateMPI(
    InType& local_matrix, const std::vector<int>& map_global_to_local, size_t pivot, size_t n,
...
size_t PetersonRGaussBandHorizontalSchemeMPI::ResolveGlobalIndex(
    const std::vector<int>& map_global_to_local, size_t local_index, size_t n) {
  for (size_t i = 0; i < n; ++i) {
    if (map_global_to_local[i] == static_cast<int>(local_index)) {
      return i;
    }
  }
  return 0;
...
std::vector<double> PetersonRGaussBandHorizontalSchemeMPI::BackSubstitutionMPI(
    const InType& local_matrix, const std::vector<int>& map_global_to_local, size_t n, size_t cols,
    int rank, int size) {
  ...
  for (int i = static_cast<int>(n) - 1; i >= 0; --i) {
    ...
    if (rank == owner) {
      ...
      result[static_cast<size_t>(i)] =
          (local_matrix[static_cast<size_t>(local_index)][cols - 1] - sum) /
