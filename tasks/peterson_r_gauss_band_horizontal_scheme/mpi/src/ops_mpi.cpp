...
void PetersonRGaussBandHorizontalSchemeMPI::SplitRows(const InType &matrix, size_t n, size_t cols, int rank, int size,
  ...
  // Исправление для clang-tidy: явно обрабатываем случай local_count == 0
  if (local_count <= 0) {
    local_matrix.clear();
    return;
  }

  // Исправление для GCC: используем resize вместо assign
  local_matrix.resize(static_cast<size_t>(local_count));
  ...
  for (size_t i = 0; i < n; ++i) {
    if (static_cast<int>(i) % size == rank) {
      ...
      if (rank == 0) {
        local_matrix[static_cast<size_t>(local_index)] = matrix[i];
      } else {
        MPI_Recv(local_matrix[static_cast<size_t>(local_index)].data(), static_cast<int>(cols), MPI_DOUBLE, 0,
...
bool PetersonRGaussBandHorizontalSchemeMPI::ForwardEliminationMPI(InType &local_matrix,
  ...
  for (size_t k = 0; k < n; ++k) {
    ...
    if (rank == owner) {
      int local_k = map_global_to_local[k];
...
void PetersonRGaussBandHorizontalSchemeMPI::EliminateMPI(InType &local_matrix,
  ...
  for (size_t i = 0; i < local_matrix.size(); ++i) {
    ...
    if (global_index > pivot && global_index < n && std::abs(local_matrix[i][pivot]) > 1e-10) {
...
size_t PetersonRGaussBandHorizontalSchemeMPI::ResolveGlobalIndex(const std::vector<int> &map_global_to_local,
  ...
  // Исправление для clang-tidy: безопасное сравнение знакового и беззнакового
  const int local_index_int = static_cast<int>(local_index);
  for (size_t i = 0; i < n; ++i) {
    if (map_global_to_local[i] >= 0 && map_global_to_local[i] == local_index_int) {
      return i;
    }
  }
  return n;  // Возвращаем n как индикатор ошибки
}
