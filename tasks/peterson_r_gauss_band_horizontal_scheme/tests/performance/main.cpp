...
// Убираем проблему с перегруженной функцией, используя явное приведение типа
using CustomPerfNameType =
    std::string (*)(const ::testing::TestParamInfo<PetersonRGaussBandHorizontalSchemePerfTests::ParamType> &);
const CustomPerfNameType kPerfTestName = PetersonRGaussBandHorizontalSchemePerfTests::CustomPerfTestName;

// Убираем функцию-обертку и вызываем макрос напрямую
[[maybe_unused]] INSTANTIATE_TEST_SUITE_P(RunModeTests, PetersonRGaussBandHorizontalSchemePerfTests, kGtestValues,
                                          kPerfTestName);
