...
class PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses
 ...
 public:
  ...
  // ЯВНО ПЕРЕОПРЕДЕЛЯЕМ МЕТОД PrintFuncTestName
  static std::string PrintFuncTestNameWrapper(
      const ::testing::TestParamInfo<typename PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses::ParamType>
          &info) {
    return PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses::PrintFuncTestName<
        PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses>(info);
  }
...
// Теперь используем wrapper функцию вместо шаблонной
[[maybe_unused]] INSTANTIATE_TEST_SUITE_P(
    GaussEliminationTests, PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses, kGtestValues,
    PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses::PrintFuncTestNameWrapper);
