...
class PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses
 ...
 public:
  static std::string PrintTestParam(const TestType& test_param) {
    return std::to_string(std::get<0>(test_param)) + "x" +
 ...
 protected:
  void SetUp() override {
    ...
    expected_output_ = OutType(static_cast<size_t>(matrix_size));
    ...
    for (int i = 0; i < matrix_size; ++i) {
      augmented_matrix[static_cast<size_t>(i)] =
          std::vector<double>(static_cast<size_t>(matrix_size) + 1, 0.0);

      double sum = 0.0;
      for (int j = 0; j < matrix_size; ++j) {
        if (std::abs(i - j) <= band_width) {
          double value = (i == j)
                             ? static_cast<double>(matrix_size + 1)
                             : 1.0 / (std::abs(i - j) + 1.0);
          augmented_matrix[static_cast<size_t>(i)][static_cast<size_t>(j)] = value;
          if (i != j) sum += std::abs(value);
        }
      }
    ...
    for (int i = 0; i < matrix_size; ++i) {
      double b = 0.0;
      for (int j = 0; j < matrix_size; ++j) {
        b += augmented_matrix[static_cast<size_t>(i)][static_cast<size_t>(j)] *
  ...
  bool CheckTestOutputData(OutType& output_data) final {
    ...
    for (size_t i = 0; i < expected_output_.size(); ++i) {
      if (std::abs(output_data[i] - expected_output_[i]) > eps) return false;
    }
    return true;
  }
...
const auto kTestTasksList =
    std::tuple_cat(
        ppc::util::AddFuncTask<PetersonRGaussBandHorizontalSchemeMPI, InType>(
            kTestParam, PPC_SETTINGS_peterson_r_gauss_band_horizontal_scheme),
        ppc::util::AddFuncTask<PetersonRGaussBandHorizontalSchemeSEQ, InType>(
            kTestParam, PPC_SETTINGS_peterson_r_gauss_band_horizontal_scheme));

const auto kGtestValues = ppc::util::ExpandToValues(kTestTasksList);

const auto kTestName =
    PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses::PrintFuncTestName<
