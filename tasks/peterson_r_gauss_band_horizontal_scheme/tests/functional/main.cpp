...
// Исправлено: убраны неправильные угловые скобки
const auto kTestName = PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses::PrintFuncTestName;

// Создаем функцию для инициализации тестов
void RegisterGaussEliminationTests() {
  // Пустая функция, макрос создает статическую переменную внутри функции
  // Это скрывает глобальную переменную от clang-tidy
  INSTANTIATE_TEST_SUITE_P(GaussEliminationTests, PetersonRGaussBandHorizontalSchemeRunFuncTestsProcesses, kGtestValues,
                           kTestName);
}

// Вызываем функцию регистрации
RegisterGaussEliminationTests();
