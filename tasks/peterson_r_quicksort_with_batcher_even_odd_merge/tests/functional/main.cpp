...
class PetersonRQuicksortWithBatcherEvenOddMergeFuncTests
 ...
 public:
  // Этот метод требует фреймворк BaseRunFuncTests
 ...
 protected:
  ...
  // Переопределяем методы базового класса (имена фиксированы фреймворком!)
...
TEST_P(PetersonRQuicksortWithBatcherEvenOddMergeFuncTests, SortingAccuracyTest) {
  // Вызов метода из базового класса BaseRunFuncTests
...
const auto test_configuration =
    std::tuple_cat(ppc::util::AddFuncTask<PetersonRQuicksortWithBatcherEvenOddMergeMPI, InputArray>(
                       test_datasets, PPC_SETTINGS_peterson_r_quicksort_with_batcher_even_odd_merge),
...
const auto test_suite_name = PetersonRQuicksortWithBatcherEvenOddMergeFuncTests::PrintFuncTestName<
    PetersonRQuicksortWithBatcherEvenOddMergeFuncTests>;

INSTANTIATE_TEST_SUITE_P(SortingVerification, PetersonRQuicksortWithBatcherEvenOddMergeFuncTests, gtest_parameters,
                         test_suite_name);
