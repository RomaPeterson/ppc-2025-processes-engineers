...
bool PetersonRQuicksortWithBatcherEvenOddMergeMPI::RunImpl() {
  ...
  // [5] Построение сети компараторов Бэтчера
...
void PetersonRQuicksortWithBatcherEvenOddMergeMPI::CreateComparatorSet(
  ...
  std::vector<int> process_ids(static_cast<size_t>(process_count_));
  for (int i = 0; i < process_count_; i++) {
    process_ids[static_cast<size_t>(i)] = i;
  }
...
void PetersonRQuicksortWithBatcherEvenOddMergeMPI::ConstructMergeSequence(
  ...
  while (!task_stack.empty()) {
    ...
    // Фаза слияния: соединяем соседние элементы
...
void PetersonRQuicksortWithBatcherEvenOddMergeMPI::ExecuteComparisons(
  ...
  for (const auto &comp : comparators) {
    ...
    const bool keep_min = (process_id_ == std::min(p1, p2));
...
void PetersonRQuicksortWithBatcherEvenOddMergeMPI::CombineSegments(const std::vector<double> &local_values,
  ...
  if (select_minimum) {
    std::copy(full_merge.begin(), full_merge.begin() + static_cast<std::ptrdiff_t>(local_size), merged_result.begin());
  } else {
    std::copy(full_merge.end() - static_cast<std::ptrdiff_t>(local_size), full_merge.end(), merged_result.begin());
  }
}
